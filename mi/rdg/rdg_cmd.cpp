...
static RHIBindPipelineParametersDesc UploadShaderParams(RDGShader * shader, RDGBuffer * shader_global_ub, RHICommandQueueGraphics & queue, const RDGShaderParamStructAndSizeInfo * base_info, void * params) {
    ...
    if (num_uniform_buffers) {
        ...
        for (int i = 0; i < num_ref_uniform_buffers; i++) {
            auto buffer_ptr = static_cast<RDGBuffer*>((void*)((uint8_t*)params + base_info->uniform_buffers_[i].cpp_offset));
            uint32_t remapped_index = shader->ConvertParamResourceIndexToBinding<RHIParamType::kUniformBuffer>((int)i);
            ret.uniforms[i] = {buffer_ptr->GetRHI(), remapped_index};
        }
        if (has_globals) {
            uint32_t remapped_index = shader->ConvertParamResourceIndexToBinding<RHIParamType::kUniformBuffer>(num_ref_uniform_buffers);
            ret.uniforms[num_ref_uniform_buffers] = {shader_global_ub->GetRHI(), remapped_index};
        }
    }
    // Bind storage buffers
    {
        ...
        for (auto [i, e] : std::views::enumerate(base_info->storage_buffers_)) {
            ...
            uint32_t remapped_index = shader->ConvertParamResourceIndexToBinding<RHIParamType::kStorageBuffer>((int)i);
            ret.storages[i] = {buffer_ptr->GetRHI(), remapped_index};
        }
    }
    // Bind UAVs
    {
        ...
        for (auto [i, e] : std::views::enumerate(base_info->uavs_)) {
            ...
            uint32_t remapped_index = shader->ConvertParamResourceIndexToBinding<RHIParamType::kUAVTexture>((int)i);
            ret.uavs[i] = {texture_ptr->GetRHI(), remapped_index};
        }
    }
    // Bind SRVs
    {
        ...
        for (auto [i, e] : std::views::enumerate(base_info->srvs_)) {
            auto texture_ptr = static_cast<RDGTexture*>((void*)((uint8_t*)params + e.cpp_offset));
            uint32_t remapped_index = shader->ConvertParamResourceIndexToBinding<RHIParamType::kSRVTexture>((int)i);
            ret.srvs[i] = {texture_ptr->GetRHI(), remapped_index};
        }
    }
...
void RDGCommandHelper::Dispatch(RHICommandQueueGraphics & queue, RDGPass * pass, RDGShader * shader,
    ...
    auto desc = UploadShaderParams(shader, pass->uniform_buffer_.Raw(), queue, info, params);
