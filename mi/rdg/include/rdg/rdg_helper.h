...
public:
    ...
    struct BufferPtrOrUint {
        union {
            RDGBuffer * buffer;
            uint32_t uint_value;
        };
        bool is_buffer;
        FORCEINLINE BufferPtrOrUint(uint32_t v) : uint_value(v), is_buffer(false) {}
        FORCEINLINE BufferPtrOrUint(RDGBuffer * buf) : buffer(buf), is_buffer(true) {}
    };
