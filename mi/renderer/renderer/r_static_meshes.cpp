...
public:
    BEGIN_SHADER_PARAMETERS(Params)
        ...
        SHADER_RESOURCE_PARAMETER(StructuredBuffer, RenderableIndexAndDescriptorIndex)
    ...
    for (auto & e : ctx.visible_renderables) {
        if (auto mesh_instance = e->As<StaticMeshInstance>()) {
            ...
            for (const auto& [descriptor_index, tup] : std::views::zip(mesh->GetGeometries(), mesh->GetMaterials()) | std::views::enumerate) {
                ...
                header.descriptor_index = descriptor_index;
...
public:
    BEGIN_SHADER_PARAMETERS(Params)
        ...
        SHADER_RESOURCE_PARAMETER(Texture2D, Visibility)
        ...
        SHADER_RESOURCE_PARAMETER(Texture2D, Depth)
    END_SHADER_PARAMETERS()
    RDG_SHADER_USE_PARAMETERS(Params)
    DECLARE_SHADER()
...
IMPLEMENT_RDG_COMPUTE_SHADER(DecodeVisibilityShader, "mi/renderer/shaders/DecodeVisibility.hlsl", "DecodeVisibility");
...
void Renderer::Render_DrawStaticMeshes(RendererView *view, RenderGraphBuilder &builder) {
    ...
    params->RenderableIndexAndDescriptorIndex = ctx.static_meshes.d_static_mesh_draw_command_renderable_descriptor_indices.Raw();
