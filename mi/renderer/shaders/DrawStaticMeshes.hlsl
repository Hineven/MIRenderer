...
StructuredBuffer<uint2>            RenderableIndexAndDescriptorIndex;
...
struct VS_Output {
    float4 Position : SV_POSITION;
    uint   DescriptorPrimitiveIndex : TEXCOORD0; // Packed DescriptorIndex (8bits) PrimitiveIndex (24bits)
    float2 UV : TEXCOORD0;
};
...
VS_Output VS_Main (DefaultStaticMeshVertex Vertex, uint InstanceIndex : SV_InstanceID) {
    ...
    Output.Position = PositionW;
    Output.UV = Vertex.UV;
    Output.DescriptorPrimitiveIndex = InstanceIndex;
    Output.MaterialIndex = RenderableIndexAndMaterialIndex[InstanceIndex].y;
    Output.WorldPosition = WorldPosition;
    return Output;
}
