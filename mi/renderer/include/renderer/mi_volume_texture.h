...
//
// CPU binary mirror: same model as Texture — the constructor allocates `data_`
// zero-filled (width * height * depth * bpp * layers) and the mirror is always
// uploaded on UpdateOnDevice. Writes via SetBinary / SetBinaryForLayer /
// GetBinaryMutable mark it dirty.
...
public:
    ...
    // Create and immediately fill the CPU mirror from `data` (whole texture).
    static TRef<VolumeTexture> CreateFromBinary(PixelFormatType format,
        uint32_t width, uint32_t height, uint32_t depth,
        std::span<uint8_t> data, uint32_t mip_levels = 1, uint32_t array_layers = 1) {
        auto tex = Create(RHITextureType::k3D, format, width, height, depth, mip_levels, array_layers);
        tex->SetBinary(data);
        return tex;
    }
    ...
    const std::vector<uint8_t> & GetBinary () const { return data_; }
    ...
    void SetBinary (std::span<uint8_t> data) {
        ...
        if (data.size() != needed_size) {
            MI_WARN("VolumeTexture::SetBinary(): Incorrect data size. Expected {}, got {}.", needed_size, data.size());
            return;
        }
    ...
    void SetBinaryForLayer (uint32_t layer, std::span<uint8_t> data) {
        ...
        if (data.size() != needed_size) {
            MI_WARN("VolumeTexture::SetBinaryForLayer(): Incorrect data size. Expected {}, got {}.", needed_size, data.size());
            return;
        }
