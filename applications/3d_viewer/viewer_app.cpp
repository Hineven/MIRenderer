...
public:
    ...
    struct FrameInternalDelayedOps {
        ...
        bool should_process_click_select {}; // Export visibility and forward depth for click selection
    ...
    void HandleNavigationInput (float delta_time) {
        ...
        if (glfwGetMouseButton(window_, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS && !input_state_.dragging_ && !ImGui::GetIO().WantCaptureMouse) {
    ...
    struct ConsoleState {
        ...
        std::vector<ConsoleLogEntry> logs;
        void ClearLog () {
            logs.clear();
        }
        void ClearHistory () {
    ...
    void HandleCVarConsole () {
	    ImGui::SetNextWindowSize(ImVec2(800, 600), ImGuiCond_FirstUseEver);
	    if (!ImGui::Begin("Console", &console_state_.opened, ImGuiWindowFlags_MenuBar))
	    {
		    ImGui::End();
		    return;
	    }
	    ...
	    if (ImGui::BeginMenuBar())
	    {
		    if (ImGui::BeginMenu("Edit"))
		    {
			    ...
			    if (clearLog || clearAll)
				    console_state_.ClearLog();
			    if (clearHistory || clearAll)
				    console_state_.ClearHistory();
			    ImGui::EndMenu();
		    }
		    ImGui::EndMenuBar();
	    }
	    ...
	    for (auto const& item : console_state_.logs)
	    {

		    bool showItem = true;
		    switch (item.type)
		    {
		    case MIInfraLogType::kInfo: showItem = console_state_.show_info; break;
		    case MIInfraLogType::kWarning: showItem = console_state_.show_warnings; break;
		    case MIInfraLogType::kError: showItem = console_state_.show_errors; break;
		    default: break;
		    }
	    ...
	    if (console_state_.scroll_to_bottom || (console_state_.auto_scroll && ImGui::GetScrollY() >= ImGui::GetScrollMaxY()))
	    {
		    ImGui::SetScrollHereY(1.f);
	    }

	    console_state_.scroll_to_bottom = false;
	    ImGui::PopStyleVar();
	    ...
	    if (ImGui::InputText("##Command", console_state_.input_buffer.data(), console_state_.input_buffer.size(), flags,
	    ...
	    {
		    if (console_state_.input_buffer.front()!='0')
		    {
			    ExecConsoleCommand(console_state_.input_buffer.data());
			    console_state_.input_buffer.front() = 0;
		    }
		    reclaim_focus = true;
	    }
	    ...
	    auto filterButton = [](char const* label, bool* value, MIInfraLogType type) {
		    ImGui::PushStyleColor(ImGuiCol_Border, GetConsoleTextColor(type));
		    ImGui::Checkbox(label, value);
		    ImGui::PopStyleColor();
	    };
	    filterButton("Error", &console_state_.show_errors, MIInfraLogType::kError); ImGui::SameLine();
	    filterButton("Warning", &console_state_.show_warnings, MIInfraLogType::kWarning); ImGui::SameLine();
	    filterButton("Info", &console_state_.show_info, MIInfraLogType::kInfo);
