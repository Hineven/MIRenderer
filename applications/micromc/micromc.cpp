...
void Start (std::unique_ptr<MIInfraInterface> && infra, const MainLoopStartConfig & cfg) {
    ...
    std::unique_ptr<MicroMCWorld> micromc_world;
    {
        // create a minecraft world and load chunks from disk
        micromc_world = std::make_unique<MicroMCWorld>();
        micromc_world->LoadFromDirectory(
            "C:\\Users\\hineven\\CLionProjects\\MIRendererDev\\applications\\micromc\\scripts\\block.json",
            "E:\\Hineven\\mc\\.minecraft\\versions\\1.20.6\\1.20.6\\assets\\minecraft\\textures\\block",
            world.get()
        );
    }
...

    {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // Render
            {
                MicroMCRenderFrame(view.get(), pool.Raw());
            }
    ...
    {
        // Release the world and its resources
        micromc_world.reset();
    }
